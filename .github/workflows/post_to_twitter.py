...
def format_tweet(post_info: Dict, max_length: int = 280) -> str:
    ...
    url_len = 23  # Twitter counts all URLs as 23 chars
    suffix = f"\n\n{url}"
    if hashtag_string:
        suffix += f"\n\n{hashtag_string}"

    available = max_length - url_len - len(suffix) + len(url)
    header = f"{title}\n\n"
    body = abstract if len(header) + len(abstract) <= available else abstract[:available - len(header) - 3] + "..."

    return header + body + suffix


def post_tweet(client: tweepy.Client, text: str, file_path: str) -> bool:
    print(f"Tweet preview ({len(text)} chars):\n{text}\n", file=sys.stderr)
    try:
        response = client.create_tweet(text=text)
        tweet_id = response.data["id"]
        print(f"✓ Posted to Twitter: https://x.com/i/web/status/{tweet_id}", file=sys.stderr)
        return True
...
def main():
    ...
    for file_path, post_info in posts.items():
        ...
        elif post_tweet(client, tweet, file_path):
            successful_posts.append(file_path)
        else:
